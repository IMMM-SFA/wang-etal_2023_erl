...
contains
  ...
  subroutine UrbanFluxes (bounds, num_nourbanl, filter_nourbanl,                        &
   ...
   !  real(r8) :: ft_greenroof(bounds%begl:bounds%endl)              ! green roof adjusting factor for temperature
    ...
    associate(                                                                & 
         ...
         fwet_road_imperv    =>   waterstate_inst%fwet_road_imperv_lun      , & ! Output: [real(r8) (:)   ]  fraction of imperv surface that is wet (-)
         ...
         z_d_town_u          =>   frictionvel_inst%z_d_town_u_lun           , & ! Input:  [real(r8) (:)   ]  
         z_0_town_u          =>   frictionvel_inst%z_0_town_u_lun           , & ! Input:  [real(r8) (:)   ]  
         obu_u               =>   frictionvel_inst%obu_u_lun                , & ! Input:  [real(r8) (:)   ]  
         zeta_u              =>   frictionvel_inst%zeta_u_lun               , & ! Input:  [real(r8) (:)   ]  
         ustar_u             =>   frictionvel_inst%ustar_u_lun              , & ! Input:  [real(r8) (:)   ]  
         tstar_u             =>   frictionvel_inst%tstar_u_lun              , & ! Input:  [real(r8) (:)   ]  
         va_u                =>   frictionvel_inst%va_u_lun                 , & ! Input:  [real(r8) (:)   ]  
      ...
      do iter = 1,niters
         ...
         do fc = 1,num_urbanc
            ...
            if (ctype(c) == icol_roof) then
               ...
               if (snow_depth(c) > 0._r8) then
                  fwet_road_imperv(l) = min(snow_depth(c)/0.05_r8, 1._r8)
               else
                  fwet_road_imperv(l) = (max(0._r8, h2osoi_liq(c,1)+h2osoi_ice(c,1))/pondmx_urban)**0.666666666666_r8
                  fwet_road_imperv(l) = min(fwet_road_imperv(l),1._r8)
               end if
               if (qaf(l) > qg(c)) then 
                  fwet_road_imperv(l) = 1._r8
               end if

               ! scaled latent heat conductance
               wtuq(c) = fwet_road_imperv(l)*(1._r8-wtroad_perv(l))*(1._r8-wtlunit_roof(l))/canyon_resistance(l)
               wtuq_road_imperv(l) = wtuq(c)
               ! unscaled latent heat conductance
               wtuq_road_imperv_unscl(l) = fwet_road_imperv(l)*(1._r8/canyon_resistance(l))
         ...
         do fl = 1, num_urbanl
            ...
            taf2(l) = (taf_numer(l)+Qanthro2/(forc_rho(g)*cpair))/taf_denom(l)
         ...
         do fl = 1, num_urbanl
            ...
            z_d_town_u(l) = z_d_town(l)
            z_0_town_u(l) = z_0_town(l)
            obu_u(l) = obu(l)
            zeta_u(l) = zeta
            ustar_u(l) = ustar(l)
            tstar_u(l) = tstar
  ...
  !----------------------------------------------------------------------- 
    ...
    associate(&
     ...
     canyon_hwr    =>    lun%canyon_hwr                 , & ! Input:  [real(r8) (:)]    ratio of building height to street width 
     wtlunit_roof  =>    lun%wtlunit_roof               , & ! Input:  [real(r8) (:)]    weight of roof with respect to landunit
     t_roof_innerl  =>   temperature_inst%t_roof_innerl_lun          , & ! Input:  [real(r8) (:)]    temp of inner layer of roof (K)
