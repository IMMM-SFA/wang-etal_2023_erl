...
contains
  ...
  subroutine SoilThermProp (bounds,  num_nolakec, filter_nolakec, &
    ...
    associate(                                                 & 
         ...
         thk_road_perv =>   soilstate_inst%thk_road_perv_col , & ! Output: [real(r8) (:,:) ]  thermal conductivity of road_perv  [W/m-K]
         cv_road_perv  =>   soilstate_inst%cv_road_perv_col    & ! Output: [real(r8) (:,:) ]  heat capacity of road_perv (J/m**3/Kelvin)         
         )
      ...
      do j = -nlevsno+1,nlevgrnd
         do fc = 1, num_nolakec
            ...
            if (j >= 1) then    
               ...
               if (col%itype(c) == icol_road_perv) thk_road_perv = thk(c,j)
            endif
      ...
      do j = 1, nlevgrnd
         do fc = 1,num_nolakec
            ...
            if (col%itype(c) == icol_road_perv) cv_road_perv = cv(c,j) / dz(c,j)
            if (j == 1) then
